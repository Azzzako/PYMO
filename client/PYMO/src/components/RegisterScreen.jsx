...
const RegisterScreen = () => {
  ...
  const searchOneHospital = () => {
    const search = hospitals.find((hos) => hos.hospitalName === searchHospital);
    if (search) {
      setResponse({
        searchResult: search,
        openSuccess: true,
  ...
  return (
    <div className="flex flex-col-reverse md:flex-row-reverse w-full h-full bg-bg3 bg-cover">
      <SearchModal response={response} setResponse={setResponse} />
      <ModalError response={response} setResponse={setResponse} />
      <div className="flex flex-col justify-center items-center w-full md:w-[50%] p-12 gap-20">
        ...
        <div className="w-full flex flex-col">
          ...
          <TextField
            ...
            onChange={(e) => setSearchHospital(e.target.value.toUpperCase())}
          ...
          />
          <div className="pt-6">
            <CustomButton1 label="Buscar" onClick={searchOneHospital} />
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col justify-center absolute w-full h-full">
          <div className="flex flex-col w-full h-full justify-center items-center p-10 lg:p-20 bg-white bg-opacity-5">
            ...
            <h1 className="font-filson text-[35px] sm:text-[35px] xl:text-[56px]">
              Ayudemos en la Lucha contra el COVID-19<br/><br/>
            </h1>
